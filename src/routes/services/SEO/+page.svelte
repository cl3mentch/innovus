<div class="lg:max-w-[1300px] m-auto mb-52 p-5 lg:p-0">
	<p class="service text-sm font-semibold mt-5">S E R V I C E</p>
	<div class="lg:flex justify-between my-10">
		<p class="text-5xl font-bold w-1/2">Search Engine <br /> Optimization</p>
		<p class="lg:w-1/2">
			Are you ready to elevate your online presence and drive your business towards success? Look no
			further than INNOVUS. We specialize in creating cutting-edge websites that not only captivate
			audiences but also deliver tangible results. <br /><br /> Our team of skilled developers, designers,
			and digital strategists work tirelessly to bring your visions to life and make your online dreams
			a reality.
		</p>
	</div>
	<div class="lg:flex justify-between mt-44">
		<div class="lg:w-1/2 lg:mr-10">
			<div
				class="bg-gradient-to-r from-[#51C5D4] to-[#2BC1E0] max-w-[150px] rounded-lg text-center py-2 text-white text-sm"
			>
				S E O <span class="ml-2">S E R V I C E</span>
			</div>
			<p class="text-4xl my-5 font-semibold leading-[1.3em]">
				Unlocking Your Online Potential & Elevate Your Presence with <br />Expert SEO Services
			</p>
			<p class="mb-3 leading-9">
				In today's competitive digital landscape, having a stunning website is only the first step.
				To truly stand out and reach your target audience, search engine optimization (SEO) is
				imperative. At Innovus, we don't just build websites; we optimize them for maximum
				visibility, higher search rankings, and increased organic traffic.
			</p>
		</div>
		<!-- svelte-ignore a11y-img-redundant-alt -->
		<img
			class="lg:w-1/2"
			src="https://firebasestorage.googleapis.com/v0/b/innovus-cd769.appspot.com/o/callcenter.jpg?alt=media&token=f37448e7-0278-41fc-9718-0e0c3baf9f5d"
			alt=""
		/>
	</div>
	<div class="my-32">
		<p class="text-4xl font-bold text-center">
			Why Choose Our <span class="text-[#2EC1DF]">SEO Services</span>?
		</p>
		<p class="text-center max-w-1/2 m-auto my-5 text-lg">
			Our team of seasoned experts is dedicated to propelling your online presence to new heights.
			With a proven track record of driving organic growth and boosting visibility, we tailor our
			strategies to your unique needs. Experience transparent reporting, data-driven insights, and a
			commitment to your success.
		</p>
		<div class="subbg-color rounded-xl lg:flex mt-20 overflow-hidden lg:h-[400px]">
			<div class="lg:w-1/2 lg:p-10 m-auto p-5">
				<p class="text-2xl font-medium text-[#2EC1DF]">Expertise That Drives Results</p>
				<p class="my-5 lg:max-w-[550px] leading-10">
					Our seasoned SEO professionals are well-versed in the latest trends and techniques that
					ensure your website is perfectly aligned with search engine algorithms. We combine
					technical expertise with creative strategies to create a winning formula for your online
					success.
				</p>
			</div>
			<img
				src="
				https://firebasestorage.googleapis.com/v0/b/innovus-cd769.appspot.com/o/rsz_business-strategy-success-target-goals.jpg?alt=media&token=cc3af607-30c5-4483-8548-42e56de7473f"
				class="lg:w-1/2 object-cover object-center
				"
				alt=""
			/>
		</div>
		<div class="lg:flex lg:space-x-5">
			<div class="lg:w-1/2">
				<div class="subbg-color rounded-xl overflow-hidden my-5">
					<div class="h-[400px] overflow-hidden">
						<img
							src="https://firebasestorage.googleapis.com/v0/b/innovus-cd769.appspot.com/o/rsz_business-owner-working-their-strategy.jpg?alt=media&token=0e552d0e-a580-415b-b2e3-f34d1dff90cd"
							alt=""
							class="object-cover object-center w-full h-full"
						/>
					</div>
					<div class="p-5 lg:p-10 m-auto">
						<p class="text-2xl font-medium text-[#2EC1DF]">Customized strategies</p>
						<p class="mt-5 lg:max-w-[550px] leading-10">
							We understand that every business is unique. That's why we tailor our SEO strategies
							to fit your specific goals, industry, and target audience. Our personalized approach
							ensures that your website not only ranks higher but also attracts the right kind of
							visitors.
						</p>
					</div>
				</div>
				<div class="subbg-color rounded-xl overflow-hidden my-5 p-5 lg:p-10">
					<div class="h-[100px] overflow-hidden">
						<img
							src="https://cdn3d.iconscout.com/3d/premium/thumb/optimization-6853705-5625772.png?f=webp"
							alt=""
							class="object-contain object-left w-full h-full"
						/>
					</div>
					<div class="lg:mt-5 m-auto">
						<p class="text-2xl font-medium text-[#2EC1DF]">On-Page and Off-Page Optimization</p>
						<p class="mt-5 leading-10">
							Our comprehensive SEO approach covers both on-page and off-page optimization. From
							optimizing your website's structure, content, and keywords to building high-quality
							backlinks, we leave no stone unturned to enhance your website's authority and
							relevance.
						</p>
					</div>
				</div>
			</div>
			<div class="lg:w-1/2">
				<div class="subbg-color rounded-xl overflow-hidden my-5 p-5 lg:p-0">
					<div class="h-[400px] overflow-hidden">
						<img
							src="https://firebasestorage.googleapis.com/v0/b/innovus-cd769.appspot.com/o/rsz_closeup-view-handshake-two-businessmen-suits-shaking-hands.jpg?alt=media&token=b311ac85-cffa-4cdc-9b15-b82fa67ed6e2"
							alt=""
							class="object-cover object-center w-full h-full"
						/>
					</div>
					<div class="lg:p-10 m-auto">
						<p class="text-2xl font-medium text-[#2EC1DF]">Transparent Collaboration</p>
						<p class="my-5 mb-10 lg:max-w-[550px] leading-10">
							We believe in transparent communication and collaboration. Throughout the SEO process,
							we keep you updated on our progress, discuss strategies, and make adjustments based on
							your feedback and evolving market trends.
						</p>
					</div>
				</div>
				<div class="subbg-color rounded-xl overflow-hidden my-5 p-5 lg:p-10">
					<div class="h-[100px] overflow-hidden">
						<img src="/data.png" alt="" class="object-contain object-left w-full h-full" />
					</div>
					<div class="lg:mt-5 m-auto">
						<p class="text-2xl font-medium text-[#2EC1DF]">Data-Driven Insights</p>
						<p class="mt-5 leading-10">
							Our comprehensive SEO approach covers both on-page and off-page optimization. From
							optimizing your website's structure, content, and keywords to building high-quality
							backlinks, we leave no stone unturned to enhance your website's authority and
							relevance.
						</p>
					</div>
				</div>
			</div>
		</div>
	</div>
	<div>
		<div>
			<p class="text-4xl font-bold text-center">
				The Importance of <span class="text-[#2EC1DF]">SEO</span>
			</p>
			<p class="text-center max-w-1/2 m-auto my-5 text-lg">
				In the ever-evolving digital landscape, search engine optimization (SEO) has emerged as a
				crucial strategy for businesses looking to establish a strong online presence and achieve
				sustainable growth. SEO isn't just a buzzword; it's a fundamental approach that directly
				impacts your website's visibility, traffic, and ultimately, your bottom line. Here's why
				investing in SEO is a strategic imperative for any business
			</p>
			<div class="grid lg:grid-cols-3 gap-4 gap-y-20 text-center icon-color mt-20">
				<div>
					<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" class="h-20 m-auto"
						><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
							d="M192 96a48 48 0 1 0 0-96 48 48 0 1 0 0 96zm-8 352V352h16v96H184zm-64 0H32c-17.7 0-32 14.3-32 32s14.3 32 32 32H152h80H608c17.7 0 32-14.3 32-32s-14.3-32-32-32H264V256.9l28.6 47.5c9.1 15.1 28.8 20 43.9 10.9s20-28.8 10.9-43.9l-58.3-97c-17.4-28.9-48.6-46.6-82.3-46.6H177.1c-33.7 0-64.9 17.7-82.3 46.6l-58.3 97c-9.1 15.1-4.2 34.8 10.9 43.9s34.8 4.2 43.9-10.9L120 256.9V448zM598.6 121.4l-80-80c-12.5-12.5-32.8-12.5-45.3 0l-80 80c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L464 141.3 464 384c0 17.7 14.3 32 32 32s32-14.3 32-32V141.3l25.4 25.4c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3z"
						/></svg
					>
					<p class="font-bold text-lg mt-5 mb-2">Enhanced Visibility and Increased Traffic</p>
					<p>SEO boosts visibility, driving organic traffic from higher search rankings.</p>
				</div>
				<div>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-20 m-auto" viewBox="0 0 640 512"
						><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
							d="M272.2 64.6l-51.1 51.1c-15.3 4.2-29.5 11.9-41.5 22.5L153 161.9C142.8 171 129.5 176 115.8 176H96V304c20.4 .6 39.8 8.9 54.3 23.4l35.6 35.6 7 7 0 0L219.9 397c6.2 6.2 16.4 6.2 22.6 0c1.7-1.7 3-3.7 3.7-5.8c2.8-7.7 9.3-13.5 17.3-15.3s16.4 .6 22.2 6.5L296.5 393c11.6 11.6 30.4 11.6 41.9 0c5.4-5.4 8.3-12.3 8.6-19.4c.4-8.8 5.6-16.6 13.6-20.4s17.3-3 24.4 2.1c9.4 6.7 22.5 5.8 30.9-2.6c9.4-9.4 9.4-24.6 0-33.9L340.1 243l-35.8 33c-27.3 25.2-69.2 25.6-97 .9c-31.7-28.2-32.4-77.4-1.6-106.5l70.1-66.2C303.2 78.4 339.4 64 377.1 64c36.1 0 71 13.3 97.9 37.2L505.1 128H544h40 40c8.8 0 16 7.2 16 16V352c0 17.7-14.3 32-32 32H576c-11.8 0-22.2-6.4-27.7-16H463.4c-3.4 6.7-7.9 13.1-13.5 18.7c-17.1 17.1-40.8 23.8-63 20.1c-3.6 7.3-8.5 14.1-14.6 20.2c-27.3 27.3-70 30-100.4 8.1c-25.1 20.8-62.5 19.5-86-4.1L159 404l-7-7-35.6-35.6c-5.5-5.5-12.7-8.7-20.4-9.3C96 369.7 81.6 384 64 384H32c-17.7 0-32-14.3-32-32V144c0-8.8 7.2-16 16-16H56 96h19.8c2 0 3.9-.7 5.3-2l26.5-23.6C175.5 77.7 211.4 64 248.7 64H259c4.4 0 8.9 .2 13.2 .6zM544 320V176H496c-5.9 0-11.6-2.2-15.9-6.1l-36.9-32.8c-18.2-16.2-41.7-25.1-66.1-25.1c-25.4 0-49.8 9.7-68.3 27.1l-70.1 66.2c-10.3 9.8-10.1 26.3 .5 35.7c9.3 8.3 23.4 8.1 32.5-.3l71.9-66.4c9.7-9 24.9-8.4 33.9 1.4s8.4 24.9-1.4 33.9l-.8 .8 74.4 74.4c10 10 16.5 22.3 19.4 35.1H544zM64 336a16 16 0 1 0 -32 0 16 16 0 1 0 32 0zm528 16a16 16 0 1 0 0-32 16 16 0 1 0 0 32z"
						/></svg
					>
					<p class="font-bold text-lg mt-5 mb-2">Credibility and Trust</p>
					<p>Top search results build credibility, trust, and brand reliability.</p>
				</div>
				<div>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-20 m-auto" viewBox="0 0 576 512"
						><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
							d="M312 24V34.5c6.4 1.2 12.6 2.7 18.2 4.2c12.8 3.4 20.4 16.6 17 29.4s-16.6 20.4-29.4 17c-10.9-2.9-21.1-4.9-30.2-5c-7.3-.1-14.7 1.7-19.4 4.4c-2.1 1.3-3.1 2.4-3.5 3c-.3 .5-.7 1.2-.7 2.8c0 .3 0 .5 0 .6c.2 .2 .9 1.2 3.3 2.6c5.8 3.5 14.4 6.2 27.4 10.1l.9 .3c11.1 3.3 25.9 7.8 37.9 15.3c13.7 8.6 26.1 22.9 26.4 44.9c.3 22.5-11.4 38.9-26.7 48.5c-6.7 4.1-13.9 7-21.3 8.8V232c0 13.3-10.7 24-24 24s-24-10.7-24-24V220.6c-9.5-2.3-18.2-5.3-25.6-7.8c-2.1-.7-4.1-1.4-6-2c-12.6-4.2-19.4-17.8-15.2-30.4s17.8-19.4 30.4-15.2c2.6 .9 5 1.7 7.3 2.5c13.6 4.6 23.4 7.9 33.9 8.3c8 .3 15.1-1.6 19.2-4.1c1.9-1.2 2.8-2.2 3.2-2.9c.4-.6 .9-1.8 .8-4.1l0-.2c0-1 0-2.1-4-4.6c-5.7-3.6-14.3-6.4-27.1-10.3l-1.9-.6c-10.8-3.2-25-7.5-36.4-14.4c-13.5-8.1-26.5-22-26.6-44.1c-.1-22.9 12.9-38.6 27.7-47.4c6.4-3.8 13.3-6.4 20.2-8.2V24c0-13.3 10.7-24 24-24s24 10.7 24 24zM568.2 336.3c13.1 17.8 9.3 42.8-8.5 55.9L433.1 485.5c-23.4 17.2-51.6 26.5-80.7 26.5H192 32c-17.7 0-32-14.3-32-32V416c0-17.7 14.3-32 32-32H68.8l44.9-36c22.7-18.2 50.9-28 80-28H272h16 64c17.7 0 32 14.3 32 32s-14.3 32-32 32H288 272c-8.8 0-16 7.2-16 16s7.2 16 16 16H392.6l119.7-88.2c17.8-13.1 42.8-9.3 55.9 8.5zM193.6 384l0 0-.9 0c.3 0 .6 0 .9 0z"
						/></svg
					>
					<p class="font-bold text-lg mt-5 mb-2">Cost-Effectiveness</p>
					<p>Unpaid organic traffic yields superior ROI over traditional marketing.</p>
				</div>
				<div>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-20 m-auto" viewBox="0 0 640 512"
						><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
							d="M48 48h88c13.3 0 24-10.7 24-24s-10.7-24-24-24H32C14.3 0 0 14.3 0 32V136c0 13.3 10.7 24 24 24s24-10.7 24-24V48zM175.8 224a48 48 0 1 0 0-96 48 48 0 1 0 0 96zm-26.5 32C119.9 256 96 279.9 96 309.3c0 14.7 11.9 26.7 26.7 26.7h56.1c8-34.1 32.8-61.7 65.2-73.6c-7.5-4.1-16.2-6.4-25.3-6.4H149.3zm368 80c14.7 0 26.7-11.9 26.7-26.7c0-29.5-23.9-53.3-53.3-53.3H421.3c-9.2 0-17.8 2.3-25.3 6.4c32.4 11.9 57.2 39.5 65.2 73.6h56.1zm-89.4 0c-8.6-24.3-29.9-42.6-55.9-47c-3.9-.7-7.9-1-12-1H280c-4.1 0-8.1 .3-12 1c-26 4.4-47.3 22.7-55.9 47c-2.7 7.5-4.1 15.6-4.1 24c0 13.3 10.7 24 24 24H408c13.3 0 24-10.7 24-24c0-8.4-1.4-16.5-4.1-24zM464 224a48 48 0 1 0 0-96 48 48 0 1 0 0 96zm-80-32a64 64 0 1 0 -128 0 64 64 0 1 0 128 0zM504 48h88v88c0 13.3 10.7 24 24 24s24-10.7 24-24V32c0-17.7-14.3-32-32-32H504c-13.3 0-24 10.7-24 24s10.7 24 24 24zM48 464V376c0-13.3-10.7-24-24-24s-24 10.7-24 24V480c0 17.7 14.3 32 32 32H136c13.3 0 24-10.7 24-24s-10.7-24-24-24H48zm456 0c-13.3 0-24 10.7-24 24s10.7 24 24 24H608c17.7 0 32-14.3 32-32V376c0-13.3-10.7-24-24-24s-24 10.7-24 24v88H504z"
						/></svg
					>
					<p class="font-bold text-lg mt-5 mb-2">Targeted Audience Reach</p>
					<p>Continuous efforts yield sustained, compounding results over time.</p>
				</div>
				<div>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-20 m-auto" viewBox="0 0 576 512"
						><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
							d="M112 48a48 48 0 1 1 96 0 48 48 0 1 1 -96 0zm40 304V480c0 17.7-14.3 32-32 32s-32-14.3-32-32V256.9L59.4 304.5c-9.1 15.1-28.8 20-43.9 10.9s-20-28.8-10.9-43.9l58.3-97c17.4-28.9 48.6-46.6 82.3-46.6h29.7c33.7 0 64.9 17.7 82.3 46.6l44.9 74.7c-16.1 17.6-28.6 38.5-36.6 61.5c-1.9-1.8-3.5-3.9-4.9-6.3L232 256.9V480c0 17.7-14.3 32-32 32s-32-14.3-32-32V352H152zm136 16a144 144 0 1 1 288 0 144 144 0 1 1 -288 0zm211.3-43.3c-6.2-6.2-16.4-6.2-22.6 0L416 385.4l-28.7-28.7c-6.2-6.2-16.4-6.2-22.6 0s-6.2 16.4 0 22.6l40 40c6.2 6.2 16.4 6.2 22.6 0l72-72c6.2-6.2 6.2-16.4 0-22.6z"
						/></svg
					>
					<p class="font-bold text-lg mt-5 mb-2">Adaptation to User Behavior</p>
					<p>Dynamic SEO adapts to trends and algorithms for continued relevance.</p>
				</div>
				<div>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-20 m-auto" viewBox="0 0 576 512"
						><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
							d="M160 0c-17.7 0-32 14.3-32 32s14.3 32 32 32h50.7L9.4 265.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L256 109.3V160c0 17.7 14.3 32 32 32s32-14.3 32-32V32c0-17.7-14.3-32-32-32H160zM576 80a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zM448 208a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zM400 384a48 48 0 1 0 0-96 48 48 0 1 0 0 96zm48 80a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zm128 0a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zM272 384a48 48 0 1 0 0-96 48 48 0 1 0 0 96zm48 80a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zM144 512a48 48 0 1 0 0-96 48 48 0 1 0 0 96zM576 336a48 48 0 1 0 -96 0 48 48 0 1 0 96 0zm-48-80a48 48 0 1 0 0-96 48 48 0 1 0 0 96z"
						/></svg
					>
					<p class="font-bold text-lg mt-5 mb-2">Competitive Edge</p>
					<p>SEO empowers with a potent competitive edge for online success..</p>
				</div>
				<div>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-20 m-auto" viewBox="0 0 512 512"
						><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
							d="M323.8 34.8c-38.2-10.9-78.1 11.2-89 49.4l-5.7 20c-3.7 13-10.4 25-19.5 35l-51.3 56.4c-8.9 9.8-8.2 25 1.6 33.9s25 8.2 33.9-1.6l51.3-56.4c14.1-15.5 24.4-34 30.1-54.1l5.7-20c3.6-12.7 16.9-20.1 29.7-16.5s20.1 16.9 16.5 29.7l-5.7 20c-5.7 19.9-14.7 38.7-26.6 55.5c-5.2 7.3-5.8 16.9-1.7 24.9s12.3 13 21.3 13L448 224c8.8 0 16 7.2 16 16c0 6.8-4.3 12.7-10.4 15c-7.4 2.8-13 9-14.9 16.7s.1 15.8 5.3 21.7c2.5 2.8 4 6.5 4 10.6c0 7.8-5.6 14.3-13 15.7c-8.2 1.6-15.1 7.3-18 15.1s-1.6 16.7 3.6 23.3c2.1 2.7 3.4 6.1 3.4 9.9c0 6.7-4.2 12.6-10.2 14.9c-11.5 4.5-17.7 16.9-14.4 28.8c.4 1.3 .6 2.8 .6 4.3c0 8.8-7.2 16-16 16H286.5c-12.6 0-25-3.7-35.5-10.7l-61.7-41.1c-11-7.4-25.9-4.4-33.3 6.7s-4.4 25.9 6.7 33.3l61.7 41.1c18.4 12.3 40 18.8 62.1 18.8H384c34.7 0 62.9-27.6 64-62c14.6-11.7 24-29.7 24-50c0-4.5-.5-8.8-1.3-13c15.4-11.7 25.3-30.2 25.3-51c0-6.5-1-12.8-2.8-18.7C504.8 273.7 512 257.7 512 240c0-35.3-28.6-64-64-64l-92.3 0c4.7-10.4 8.7-21.2 11.8-32.2l5.7-20c10.9-38.2-11.2-78.1-49.4-89zM32 192c-17.7 0-32 14.3-32 32V448c0 17.7 14.3 32 32 32H96c17.7 0 32-14.3 32-32V224c0-17.7-14.3-32-32-32H32z"
						/></svg
					>
					<p class="font-bold text-lg mt-5 mb-2">User Experience Improvement</p>
					<p>SEO enhances user experience through design, content, and engagement optimization.</p>
				</div>
				<div>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-20 m-auto" viewBox="0 0 384 512"
						><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
							d="M32 0C14.3 0 0 14.3 0 32S14.3 64 32 64V75c0 42.4 16.9 83.1 46.9 113.1L146.7 256 78.9 323.9C48.9 353.9 32 394.6 32 437v11c-17.7 0-32 14.3-32 32s14.3 32 32 32H64 320h32c17.7 0 32-14.3 32-32s-14.3-32-32-32V437c0-42.4-16.9-83.1-46.9-113.1L237.3 256l67.9-67.9c30-30 46.9-70.7 46.9-113.1V64c17.7 0 32-14.3 32-32s-14.3-32-32-32H320 64 32zM96 75V64H288V75c0 19-5.6 37.4-16 53H112c-10.3-15.6-16-34-16-53zm16 309c3.5-5.3 7.6-10.3 12.1-14.9L192 301.3l67.9 67.9c4.6 4.6 8.6 9.6 12.1 14.9H112z"
						/></svg
					>
					<p class="font-bold text-lg mt-5 mb-2">Long-Term Results</p>
					<p>SEO's value compounds over time, building sustainable online presence.</p>
				</div>
				<div>
					<svg xmlns="http://www.w3.org/2000/svg" class="h-20 m-auto" viewBox="0 0 448 512"
						><!--! Font Awesome Free 6.4.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2023 Fonticons, Inc. --><path
							d="M160 80c0-26.5 21.5-48 48-48h32c26.5 0 48 21.5 48 48V432c0 26.5-21.5 48-48 48H208c-26.5 0-48-21.5-48-48V80zM0 272c0-26.5 21.5-48 48-48H80c26.5 0 48 21.5 48 48V432c0 26.5-21.5 48-48 48H48c-26.5 0-48-21.5-48-48V272zM368 96h32c26.5 0 48 21.5 48 48V432c0 26.5-21.5 48-48 48H368c-26.5 0-48-21.5-48-48V144c0-26.5 21.5-48 48-48z"
						/></svg
					>
					<p class="font-bold text-lg mt-5 mb-2">Analytics and Insights</p>
					<p>SEO offers audience insights, aiding informed marketing decisions with data.</p>
				</div>
			</div>
		</div>
	</div>
</div>

<style>
	.service::before {
		content: '';
		background-color: #3ec6ec;
		width: 50px;
		height: 2px;
		display: inline-block;
		vertical-align: middle;
		margin-right: 0.5rem;
	}
</style>
